...
def drain_queue(
    *,
    ...
    """Pull up to ``limit`` messages. ACK on success, nack/requeue on handler error."""
    if not rabbit_url:
        return 0
    processed = 0
    with Connection(rabbit_url) as conn:
        ...
        try:
            for _ in range(max(1, limit)):
                ...
                try:
                    ...
                    logger.warning("Dropping poison message on %s: %s", queue_name, exc)
                    try:
                        message.reject(requeue=False)
                    except Exception:
                        logger.exception("Failed to drop poison message on %s", queue_name)
                    processed += 1
                except Exception as exc:
                    logger.warning("RabbitMQ handler failed for %s: %s", queue_name, exc)
                    try:
                        message.reject(requeue=True)
                    except Exception:
                        logger.exception("Failed to requeue message on %s", queue_name)
                    break
        finally:
            queue.close()
    return processed
